	...
	public String decrypt(String cipherText, int key) {
		...
		for (int i = 0; i < cipherText.length(); i++) {
			...
			if (Character.isUpperCase(cipherCharacter)) {
				...
				int newPosition =(position - key) % alphabetUppercase.length();
				if (newPosition < 0) {
					newPosition = newPosition + alphabetUppercase.length();
				}
				...
				int newPosition = (position - key) % alphabetLowercase.length();
				if (newPosition < 0) {
					newPosition = newPosition+ alphabetLowercase.length();
				}
				...
				int newPosition = (position - key) % number.length();
				if (newPosition < 0) {
					newPosition = newPosition + number.length();
				}
				...
				int newPosition = (position - key) % symbol.length();
				if(newPosition <0) {
					newPosition = newPosition + symbol.length();
				}
